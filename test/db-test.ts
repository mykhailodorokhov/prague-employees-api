...
export async function destroyTestDb(fastify: FastifyInstance) {
  await fastify.excel.schema.dropTable(EMPLOYEES_TABLE);
  await fastify.excel.schema.dropTable(TRIBES_TABLE);
}
export async function createTables(fastify: FastifyInstance) {
  await fastify.excel.schema.createTable(TRIBES_TABLE, (table) => {
    table.increments("id").primary();
    table.string("name").notNullable();
    table.string("department").notNullable();
  });
  await fastify.excel.schema.createTable(EMPLOYEES_TABLE, (table) => {
    table.increments("id").primary();
    table.string("name").notNullable();
...
export async function generateData(fastify: FastifyInstance) {
  ...
  await fastify.excel.from(TRIBES_TABLE).insert(tribesSeedData);
  await fastify.excel.from(EMPLOYEES_TABLE).insert(employeesSeedData);
}
